...
from skimage.measure import approximate_polygon, label, regionprops, subdivide_polygon
...
def reading_order(
    lines: Sequence[tuple[slice, slice]], text_direction: Literal["lr", "rl"] = "lr"
) -> np.ndarray:
    ...
    logger.info(
        "Compute reading order on {} lines in {} direction".format(
            len(lines), text_direction
        )
    )
...
def _extend_boundaries(baselines, bin_bl_map):
    ...
    for x in regionprops(labelled):
        try:
            ...
            if x.area < 6:
                logger.info(
                    f"Skipping baseline extension for very small blob of area {x.area}"
                )
                continue
    ...
    for bl in baselines:
        ...
        if boundary_pol.contains(geom.Point(bl[0])):
            l_point = boundary_pol.boundary.intersection(
                geom.LineString(
                    [
                        (
                            bl[0][0] - 10 * (bl[1][0] - bl[0][0]),
                            bl[0][1] - 10 * (bl[1][1] - bl[0][1]),
                        ),
                        bl[0],
                    ]
                )
            ...
            if l_point.geom_type != "Point":
                bl[0] = np.array(
                    nearest_points(geom.Point(bl[0]), boundary_pol)[1].coords[0], "int"
                ).tolist()
            else:
        ...
        if boundary_pol.contains(geom.Point(bl[-1])):
            ...
            if r_point.geom_type != "Point":
                bl[-1] = np.array(
                    nearest_points(geom.Point(bl[-1]), boundary_pol)[1].coords[0], "int"
                ).tolist()
            else:
...
def vectorize_lines(
    ...
    line_extrema = np.transpose(
        np.where((convolve2d(line_skel, kernel, mode="same") == 11) * line_skel)
    )
    ...
    for bl in lines:
        ...
        r_end = tuple(bl[-1])
        if (
            f_st_map[l_end] - f_end_map[l_end] > 0.2
            and f_st_map[r_end] - f_end_map[r_end] < -0.2
        ):
            pass
        elif (
            f_st_map[l_end] - f_end_map[l_end] < -0.2
            ...
            if text_direction == "horizontal":
                logger.debug(
                    "Insufficient marker confidences in output. Defaulting to horizontal upright line."
                )
                if bl[0][1] > bl[-1][1]:
                    bl = bl[::-1]
            else:
                logger.debug(
                    "Insufficient marker confidences in output. Defaulting to top-to-bottom line."
                )
                if bl[0][0] > bl[-1][0]:
                    bl = bl[::-1]
...
def _rotate(
    ...
    cval: int | tuple = 0,
    order: int = 0,
    use_skimage_warp: bool = False,
) -> tuple[AffineTransform, _T_pil_or_np]:
    """
    ...
    if isinstance(image, Image.Image):
        ...
        resample = {
            ...
            3: Resampling.BICUBIC,
        }.get(order, Resampling.NEAREST)
        return tform, image.transform(
            output_shape[::-1],
            Transform.AFFINE,
            data=pdata,
            resample=resample,
...
def _calc_seam(baseline, polygon, angle, im_feats, bias=150):
    ...
    for line_seg in zip(baseline[:-1] - (c_min, r_min), baseline[1:] - (c_min, r_min)):
        line_locs = draw.line(
            line_seg[0][1], line_seg[0][0], line_seg[1][1], line_seg[1][0]
        )
        mask[line_locs] = 0
    dist_bias = distance_transform_cdt(mask)
    # absolute mask
    mask = (
        np.array(make_polygonal_mask(polygon - (c_min, r_min), patch.shape[::-1]))
        <= 128
    )
    ...
    rotated_patch = np.pad(
        rotated_patch, ((1, 1), (0, 0)), mode="constant", constant_values=np.inf
    )
...
def _extract_patch(
    ...
    roi_polygon = unary_union(
        [geom.Polygon(upper_polygon), geom.Polygon(bottom_polygon)]
    )

    if topline:
        upper_seam = _calc_seam(baseline, upper_polygon, angle, im_feats)
        bottom_seam = _calc_seam(
            offset_baseline, bottom_offset_polygon, angle, im_feats
        )
    else:
    ...
    if (
        upper_seam.parallel_offset(offset // 2, side="right").geom_type
        == "MultiLineString"
        or offset == 0
    ):
        ...
        upper_seam = np.array(
            upper_seam.parallel_offset(offset // 2, side="right").coords, dtype=int
        )[::-1]
    if (
        bottom_seam.parallel_offset(offset // 2, side="left").geom_type
        == "MultiLineString"
        or offset == 0
    ):
        bottom_seam = np.array(bottom_seam.coords, dtype=int)
    else:
        bottom_seam = np.array(
            bottom_seam.parallel_offset(offset // 2, side="left").coords, dtype=int
        )

    # offsetting might produce bounds outside the image. Clip it to the image bounds.
    polygon = np.concatenate(
        ([end_points[0]], upper_seam, [end_points[-1]], bottom_seam[::-1])
    )
    polygon = geom.Polygon(polygon)
    if not polygon.is_valid:
        polygon = np.concatenate(
            ([end_points[-1]], upper_seam, [end_points[0]], bottom_seam)
        )
        polygon = geom.Polygon(polygon)
    if not polygon.is_valid:
        raise Exception(
            f"Invalid bounding polygon computed: {explain_validity(polygon)}"
        )
...
def _calc_roi(line, bounds, baselines, suppl_obj, p_dir):
    ...
    for point, upper_bounds_intersect, bottom_bounds_intersect in zip(
        ...
        bottom_limit = _find_closest_point(
            point,
            geom.LineString([point, bottom_bounds_intersect]).intersection(side_b),
        )
...
def calculate_polygonal_environment(
    ...
    if scale is not None and (scale[0] > 0 or scale[1] > 0):
        ...
        if suppl_obj is not None:
            suppl_obj = [
                (np.array(bl) * scale).astype("int").tolist() for bl in suppl_obj
            ]
    ...
    for idx, line in enumerate(baselines):
        try:
            ...
            offset = (
                default_specs.SEGMENTATION_HYPER_PARAMS["line_width"]
            ...
            env_up, env_bottom = _calc_roi(
                line, bounds, baselines[:idx] + baselines[idx + 1 :], suppl_obj, p_dir
            )
    ...
    if scale is not None:
        polygons = [
            (np.array(pol) / scale).astype("uint").tolist() if pol is not None else None
            for pol in polygons
        ]
    return polygons
...
def polygonal_reading_order(
    ...
    """
    lines = [
        (line["tags"]["type"], line["baseline"], line["boundary"]) for line in lines
    ]
            ...
            if is_in_region(s_line, reg):
                region_lines[idx].append(
                    (
                        line_idx,
                        (
                            slice(s_line.bounds[1], s_line.bounds[3]),
...
def scale_polygonal_lines(
    lines: Sequence[tuple[list, list]], scale: Union[float, tuple[float, float]]
) -> Sequence[tuple[list, list]]:
    """
    Scales baselines/polygon coordinates by a certain factor.
    ...
    for line in lines:
        ...
        scaled_lines.append(
            (
                (np.array(bl) * scale).astype("int").tolist(),
...
def _bevelled_warping_envelope(
    baseline: np.ndarray,
    ...
    target_envelope = [(0, 0), (0, output_shape[0])]
    ...
    for k in range(l_bl - 2):
        ...
        bevel_prev = seg_prev / max(2.0, np.linalg.norm(seg_prev) / MAX_BEVEL_WIDTH)
        bevel_next = seg_next / max(2.0, np.linalg.norm(seg_next) / MAX_BEVEL_WIDTH)
        bevel_nsteps = max(
            1,
            np.round(
                (np.linalg.norm(bevel_prev) + np.linalg.norm(bevel_next))
                / BEVEL_STEP_WIDTH
            ),
        )
        ...
        for i in range(int(bevel_nsteps) + 1):
            ...
            source_points = [
                _as_int_tuple(tpt + envelope_dy[0] * tnormal),
                _as_int_tuple(tpt + envelope_dy[1] * tnormal),
            ]
            target_points = [(int(tx), 0), (int(tx), output_shape[0])]
            # avoid duplicate points leading to singularities
            if (
                source_points[0] == source_envelope[-2]
                or source_points[1] == source_envelope[-1]
                or target_points[0] == target_envelope[-2]
            ):
                continue
            source_envelope += source_points
            target_envelope += target_points

    end_point = baseline[-1] + diff_bl_normed[-1] * (
        output_shape[1] - cum_lens[-1] - output_bl_start[0]
    )
    source_envelope += [
        end_point + envelope_dy[0] * bl_seg_normals[-1],
        end_point + envelope_dy[1] * bl_seg_normals[-1],
    ]
    target_envelope += [(output_shape[1], 0), (output_shape[1], output_shape[0])]
...
def make_polygonal_mask(polygon: np.ndarray, shape: tuple[int, int]) -> Image.Image:
    ...
    ImageDraw.Draw(mask).polygon(
        [tuple(p) for p in polygon.astype(int).tolist()], fill=255, width=2
    )
    return mask


def apply_polygonal_mask(
    img: Image.Image, polygon: np.ndarray, cval: int | tuple = 0
) -> Image.Image:
    """
...
def extract_polygons(
    im: Image.Image, bounds: "Segmentation", legacy: bool = False, pad=0
) -> Generator[
    tuple[
        Image.Image,
    ...
    if bounds.type == "baselines":
        ...
        for line in bounds.lines:
            ...
            # c_min = max(c_min, 0)
            # r_min = max(r_min, 0)
            # c_max = min(c_max, im.width - 1)
            # r_max = min(r_max, im.height - 1)
            ...
            if legacy:
                ...
                if len(baseline) == 2:
                    ...
                    p_dir = np.mean(
                        np.diff(baseline.T) * lengths / lengths.sum(), axis=1
                    )
                    p_dir = p_dir.T / np.sqrt(np.sum(p_dir**2, axis=-1))
                    ...
                    # scale line image to max 600 pixel width
                    tform, rotated_patch = _rotate(
                        patch, angle, center=extrema[0], scale=1.0, cval=(255, 255, 255)
                    )
                ...
                # normal slow path with piecewise affine transformation
                    ...
                    bl_dst_pts = (
                        baseline[0] + np.dstack((cum_lens, np.zeros_like(cum_lens)))[0]
                    )
                    ...
                    c_dst_min, c_dst_max = (
                        int(pol_dst_pts[:, 0].min()),
                        int(pol_dst_pts[:, 0].max()),
                    )
                    r_dst_min, r_dst_max = (
                        int(pol_dst_pts[:, 1].min()),
                        int(pol_dst_pts[:, 1].max()),
                    )
                ...
                if len(baseline) == 2:
                    ...
                    offset_polygon = pl - (c_min, r_min)
                    patch = apply_polygonal_mask(
                        patch, offset_polygon, cval=(255, 255, 255)
                    )
                    extrema = offset_polygon[(0, -1), :]
                    tform, i = _rotate(
                        patch,
                ...
                # normal slow path with piecewise affine transformation
                    ...
                    poly_bl_segdist = np.maximum(
                        -poly_bl_x, poly_bl_x - diff_bl_norms[:, None]
                    )
                    ...
                    pol_dst_pts = (
                        np.array([cum_lens[poly_closest_bl] + poly_bl_x, poly_bl_y]).T
                        + baseline[:1]
                    )
                    ...
                    source_envelope, target_envelope = _bevelled_warping_envelope(
                        offset_baseline, offset_bl_dst_pts[0], output_shape
                    )
        ...
        for line in bounds.lines:
            ...
            if (
                box < [0, 0, 0, 0]
                or box[::2] >= [im.size[0], im.size[0]]
                or box[1::2] >= [im.size[1], im.size[1]]
            ):
                logger.error(
                    "bbox {} is outside of image bounds {}".format(box, im.size)
                )
                raise KrakenInputException("Line outside of image bounds")
            yield im.crop(box).rotate(angle, expand=True), box


def expand_boundary(seg: "Segmentation", im: Image.Image, padding=5) -> "Segmentation":
    if seg.type == "baselines":
        for j, line in enumerate(seg.lines):
            ...
            r_min_index, r_max_index = (
                np.where(pl[:, 1] == r_min)[0],
                np.where(pl[:, 1] == r_max)[0],
            )
            if len(c_max_index) >= 2:
                if np.abs(c_max_index[-1] - c_max_index[0]) != len(c_max_index) - 1:
                    continue
            for i, item in enumerate(pl):
                if i <= c_max_index[0]:
                    seg.lines[j].boundary[i][1] += padding
                    seg.lines[j].boundary[i][1] = min(
                        seg.lines[j].boundary[i][1], im.height - 1
                    )
                else:
                    seg.lines[j].boundary[i][1] -= padding
                    seg.lines[j].boundary[i][1] = max(seg.lines[j].boundary[i][1], 0)

                if i in c_max_index:
                    seg.lines[j].boundary[i][0] += 30
                    seg.lines[j].boundary[i][0] = min(
                        seg.lines[j].boundary[i][0], im.width - 1
                    )
                elif i in c_min_index:
                    seg.lines[j].boundary[i][0] -= 30
